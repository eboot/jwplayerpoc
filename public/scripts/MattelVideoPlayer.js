...
(function() {
    ...
    var screenWidth,
        ...
        playListDomain = "//content.jwplatform.com/libraries/",
    ...
    function renderOoyalaApi(callback) {
       ...
       // pCodeVal = document.getElementById("oo-player-pcode") != null ? document.getElementById("oo-player-pcode").value : [],
		...
		//hdnPlayerId = $("#oo-player-id input").val();
	...
	//bodyElementTag.appendChild(fileBinding('js',hostedDomain+"ooyala-master-plugin.js",callback));
        if((".ooyala-video-player[data-customPlayer]").length > 0 ){
            bodyElementTag.appendChild(fileBinding('js',playListDomain+brandPlayerIds+'.js',callback));
        }else
    	{
    		//bodyElementTag.appendChild(fileBinding('js',playListDomain+hdnPlayerId+'?platform=html5-priority',callback));
    	}
    ...
    **/
    ...
    function playerBinding(curEle) {
       ...
       /* if(MattelVideoPlayer.ooPlayerInstances.players.length==1){
         ...
         jwplayer("video-container-1").setup({
...
*/
    ...
    MattelVideoPlayer.init = function() {
        MattelVideoPlayer.ooyalaApiLoaded = apiLoaded = (typeof jwplayer!="undefined") ? true : false;
